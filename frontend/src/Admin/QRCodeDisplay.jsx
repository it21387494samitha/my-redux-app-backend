...
const QRCodeDisplay = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const response = await axios.get('https://sltseatbooking1.azurewebsites.net/events/allad', {
          headers: {
  ...
  const handleDeleteClick = async (eventId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`https://sltseatbooking1.azurewebsites.net/events/soft/${eventId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
