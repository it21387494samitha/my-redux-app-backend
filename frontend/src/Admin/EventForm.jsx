...
const EventForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        'https://newsltazure.azurewebsites.net/events/',
        formData,
        {
          headers: {
  ...
  const handleSeatSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        'https://newsltazure.azurewebsites.net/api/seats/creat',  // Corrected the URL here
        {
          eventId,
  ...
  useEffect(() => {
    const fetchExistingEvents = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get('https://newsltazure.azurewebsites.net/events', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
