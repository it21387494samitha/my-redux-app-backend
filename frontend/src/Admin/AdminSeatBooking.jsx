...
const AdminSeatBooking = () => {
    ...
    useEffect(() => {
        ...
        const fetchSeats = async () => {
            try {
                ...
                const response = await axios.get('https://sltseatbooking1.azurewebsites.net/api/seats/seats', {
                    headers: {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this seat?')) {
            try {
                const token = localStorage.getItem('token');
                await axios.delete(`https://sltseatbooking1.azurewebsites.net/api/seats/seats/${id}`, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                    },
                });
